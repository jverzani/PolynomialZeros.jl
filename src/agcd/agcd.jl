...
function lemma24(A::Matrix; θ::Float64=1e-8)
    const MAXSTEPS = 100
...
residual_error(p,q,u,v,w, wts=ones(degree(p) + degree(q) + 2))::Float64 = norm(Fmp(p,q,u,v,w) .* wts, 2)
...
"""
function agcd{T <: Real,S <: Real}(p::Poly{T}, q::Poly{S}=polyder(p);
              theta::Float64 = 1e-12,    # reveal_rank tolerance. (1e-8 in paper, this seems better?)
              ρ::Float64 = 1e-10   # residual tolerance
              )
